...
class DevPackageHandler extends \App\Support\PackageHandler
{
    ...
    public function makeMigrationHandle(Command $command)
    {
        ...
        $path = Path::relative(
            $this->app->basePath(),
            $this->app->make('path.package-sso.migrations')
        );
    ...
    {
        $path = $this->app->make('path.package-sso.seeds');
        ...
        $content = file_get_contents(
            $this->app->make('path.package-sso.resources').'/stubs/seeder'
        );
    ...
    public function makeModelHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.package-sso.resources').'/stubs/model'
        );
        $variable = [
            '{--name--}' => $modelName,
            '{--table-name--}' => $table,
